...
async function requireTeam(workspace: string, config: ToolsConfig, captain: Agent): Promise<TeamState> {
    ...
    async execute(args, exec) {
      ...
      return withTeamLock(team.id, async () => {
        ...
        if (isCaptain) {
          ...
          if (mine === undefined) throw new Error('only team members can claim tasks')
...
function memberNameOf(team: TeamState, agentId: string): string | undefined {
