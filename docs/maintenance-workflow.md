...
| 当前候选宿主 | 定位 | 安装与支持边界 |
| --- | --- | --- |
| `0.1.7-rc.1` | recommended | 当前普通用户默认组合的验收目标；RC 不是 GA，仍须实际验证 |

`0.1.5-rc.1` 及更早宿主不再在候选矩阵中：浏览器侧对两代宿主是结构性互斥的（`agent/session-start` 在 0.1.7 并入 serial 的 `agent/created`；`plugin` 消息来源类型被删除；`dsh-client-ui-primitives` 的图标被改名而该包由宿主模块表解析，无运行时别名；会话导航从 `ISessions` 移到 `uiWorkspace`）。这些旧宿主请继续使用插件 `0.1.20`，不要只升级一端。
...
- 无后缀插件首次发布及 RC 产物提升到 `latest` 时，开发基线必须是清单中的 recommended 宿主，全部支持目标都要通过。当前 recommended 为 `0.1.7-rc.1`，不得把只在 Alpha 通过的包提升为普通用户默认版本。
