...
[旧固定协议报告](./progressive-loading-verification.json)和[先前真实模型对照](./agent-teams-real-model-verification.json)是仍包含 open 的历史产物证据，不代表当前 13 工具版本。包版本暂保留 `0.1.16-rc.1`；本地开发包以 SHA-256 区分，npm 上的同版本旧包不包含本次修改。
