...
3. Create all tasks in ONE step: call agent_teams_create_task in parallel for every task. Task ids are sequential (t1, t2, t3...), so you can reference dependencies before they are created (e.g. t3 can depend on t1 and t2 in the same batch). Do NOT create tasks one-by-one. After creating tasks, do NOT call agent_teams_send_message to assign or dispatch tasks — create_task automatically notifies the assignee. You only need send_message for guidance/clarification, not for task dispatch.
4. Lead by delegation: let idle teammates execute ready work. Do not duplicate a teammate's work merely because its turn is slow. Members report completion to you automatically via agent_teams_send_message — when a member sends you a message, it wakes you automatically and the message appears in your next turn's inbox; you do NOT need to sleep or poll to wait for them. After creating all tasks, simply end your turn (stop calling tools, output a brief "waiting for members" text). When members complete their work, their messages will wake you automatically. Do NOT call agent_teams_status to check progress. Do NOT use bash sleep to wait for members — just end your turn and let the scheduler wake you when results arrive.
...
6. Tasks carry attempt_id capabilities. Members must use the current attempt_id for updates; stale-attempt errors mean ownership changed. Wait for member completion messages instead of polling — each member sends you a message when its task is done.
7. When all members have reported completion (or you receive their results via messages), present the team's results to the user, then call agent_teams_delete. Do NOT output a recap or summary after delete — the results you already presented are sufficient.
