...
队长会话从第一轮起保留固定核心协议和 14 个原生团队工具。`agent_teams_open` 可选地读取当前团队状态、模板用途和规划模式；原有 13 个业务工具无需先调用入口，旧工具白名单也可以直接使用。已配置模板的精简目录仍固定保留，旧白名单也能发现模板。打开或结束团队都不会改写系统提示词和工具 schema，会话压缩或代码模式丢弃工具结果也不会丢失核心规则。成员保留四个团队工具、固定成员说明及普通编程/研究工具。Web 批准后会通知并唤醒队长，后续成员报告会再次唤醒它，无需忙轮询。详见[加载机制与 benchmark 标准](./docs/progressive-loading.md)。
