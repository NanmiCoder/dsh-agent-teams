...
3. Create all tasks in ONE step: call agent_teams_create_task in parallel for every task. Task ids are sequential (t1, t2, t3...), so you can reference dependencies before they are created (e.g. t3 can depend on t1 and t2 in the same batch). Do NOT create tasks one-by-one.
4. Lead by delegation: send guidance with agent_teams_send_message and let idle teammates execute ready work. Do not duplicate a teammate's work merely because its turn is slow. Members report completion to you automatically via agent_teams_send_message — messages are injected into your current turn as inbox deliveries. After creating all tasks, call bash sleep 10 ONCE to yield control for members to run. After sleep returns, your next step will automatically contain any member messages that arrived during sleep — if you see "AgentTeams message from member" in your inbox, those members are DONE; process their results immediately and do NOT sleep again. If some members have NOT yet reported, you may sleep 10 ONE more time to wait for them. Never sleep more than twice total. Do NOT call agent_teams_status.
