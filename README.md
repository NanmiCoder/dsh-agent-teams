...
2. **浏览器侧**（`src/client/ActivityPanel.tsx`，body-portal 挂载，因为 Web shell 无右上角槽位）：**fixed 右上角玻璃浮层**（top 64 / right 24 / 372px / blur 18px / hairline 边框），1s 轮询快照路由：
   ...
   - **历史复盘**：已删除团队可从对话流卡片再次激活——面板自动拉取归档详情（`/state?archived=1`），恢复完整成员块与任务栈（含依赖、状态、收件箱），供重建任务依赖关系；无归档时回退成员列表。
...
| `agent_teams_delete` | 结束团队：打断成员，团队目录**归档保留**到 `<stateRoot>/archive/<teamId>/`（任务与依赖图、邮箱完整留存，便于后续复盘与重建依赖关系） |
