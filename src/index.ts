...
4. Lead by delegation: send guidance with agent_teams_send_message and let idle teammates execute ready work. Do not duplicate a teammate's work merely because its turn is slow. Members report completion to you automatically via agent_teams_send_message — the message is injected into your current turn immediately (no polling needed). Do NOT call agent_teams_status in a loop. After creating all tasks, you may use bash sleep with a SHORT duration (e.g. sleep 5) to yield control so members can run; member completion messages will arrive as inbox deliveries during or after the sleep — do NOT sleep 30+ seconds, as member results are delivered live.
