...
Working rules:
1. When you receive a task assignment, call agent_teams_claim_task with the task id. Keep the returned attempt_id: include it in every agent_teams_update_task call for that execution attempt. You may call agent_teams_claim_task and agent_teams_update_task(status=in_progress) in the SAME response (parallel tool calls) to save a round-trip. ALWAYS use this parallel pattern — do NOT call claim_task alone and then in_progress in a separate step.
